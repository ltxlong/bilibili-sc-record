...
(function() {
    ...
    let room_id = (new URL(window.location.href)).searchParams.get('roomid'); // 获取直播间id

    // 防止加载两次：根据两种url判断，b站默认点击跳转进入房间的会出现重复加载
    ...
    function sc_process_start() {
        ...
        const sc_circleContainer = window.parent.document.createElement('div');
        ...
        const sc_rectangleContainer = window.parent.document.createElement('div');
        ...
        const sc_minimizeButton = window.parent.document.createElement('button');
        ...
        $(window.parent.document).on('click', '.sc_button_min', sc_minimize);

        // Add a button to the page to trigger export function
        const sc_exportButton = window.parent.document.createElement('button');
        ...
        $(window.parent.document).on('click', '.sc_button_export', sc_export);

        // Add a button to the page to trigger model function
        const sc_modelButton = window.parent.document.createElement('button');
        ...
        $(window.parent.document).on('click', '.sc_button_model', sc_model_change);

        // Add a button to the page to trigger switch function
        const sc_switchButton = window.parent.document.createElement('button');
        ...
        $(window.parent.document).on('click', '.sc_button_switch', sc_switch_css);

        // Create a container for the buttons
        const sc_buttonsContainer = window.parent.document.createElement('div');
        ...
        const sc_dataShowContainer = window.parent.document.createElement('div');
        ...
        const sc_label_high_energy_num_left = window.parent.document.createElement('label');
        ...
        const sc_label_high_energy_num_right = window.parent.document.createElement('label');
        ...
        const sc_label_captain_num_left = window.parent.document.createElement('label');
        ...
        const sc_label_captain_num_right = window.parent.document.createElement('label');
        ...
        const sc_listContainer = window.parent.document.createElement('div');
        ...
        // scrollbar css
        let sc_scrollbar_style = window.parent.document.createElement('style');
        ...
        `;
        window.parent.document.head.appendChild(sc_scrollbar_style);

        let sc_other_style = window.parent.document.createElement('style');
        ...
        `;
        window.parent.document.head.appendChild(sc_other_style);


        let live_player_div = window.parent.document.getElementById('live-player');
        let live_room_div = window.parent.document.getElementsByClassName('live-room-app')[0];
        ...
        sc_circleContainer.style.top = `${window.parent.innerHeight / 4}px`;

        $(window.parent.document).on('mousedown', '.sc_drag_div', sc_startDragging);
        $(window.parent.document).on('mousemove', sc_drag);
        $(window.parent.document).on('mouseup', sc_stopDragging);
        ...
        function sc_drag(e) {
            if (sc_isDragging) {
                let sc_elements = $(window.parent.document).find('.sc_drag_div');
                sc_elements.each(function(){
                    const rect = this.getBoundingClientRect();

                    const maxX = window.parent.innerWidth - rect.width;
                    const maxY = window.parent.innerHeight - rect.height;
        ...
        $(window.parent.document).on('click', '.sc_circle', (e) => {
            if (sc_isClickAllowed) {
                let xPos = 0;
                let yPos = 0;
                let sc_circles = $(window.parent.document).find('.sc_circle');
                ...
                if (window.parent.innerWidth - xPos < 300) {
                    xPos = window.parent.innerWidth - 305;
                }

                if (window.parent.innerHeight - yPos < sc_panel_high) {
                    yPos = window.parent.innerHeight - sc_panel_high - 5;
                }

                let sc_rectangles = $(window.parent.document).find('.sc_rectangle');
                sc_rectangles.each(function() {
                    $(this).css('left', xPos + 'px');
                    $(this).css('top', yPos + 'px');

                    $(this).slideDown(500, function () {
                        $(window.parent.document).find('.sc_buttons').slideDown(500);
                    });
                });
        ...
        $(window.parent.document).on('mouseenter', '.sc_circle', (e) => {
            $(window.parent.document).find('.sc_circle').css('border', '3px solid rgba(255,255,255,0.5)');
        });

        $(window.parent.document).on('mouseleave', '.sc_circle', (e) => {
            $(window.parent.document).find('.sc_circle').css('border', '2px solid #ffffff');
        });

        let sc_rectangle_is_slide_down = false;
        let sc_rectangle_is_slide_up = false;
        // 优化回弹问题
        $(window.parent.document).on('mouseenter', '.sc_rectangle', (e) => {
            if (sc_rectangle_is_slide_down) {
                return;
            }
            sc_rectangle_is_slide_down = true;

            $(window.parent.document).find('.sc_buttons').slideDown(500, function() {
                sc_rectangle_is_slide_down = false;
            });
            $(window.parent.document).find('.sc_data_show').slideDown(500, function() {
                sc_rectangle_is_slide_down = false;
            });
            $(window.parent.document).find('.sc_data_show label').animate({opacity: 1}, 1000);

        });

        $(window.parent.document).on('mouseleave', '.sc_rectangle', (e) => {
            if (sc_rectangle_is_slide_up) {
                return;
            }
            sc_rectangle_is_slide_up = true;

            $(window.parent.document).find('.sc_buttons').slideUp(500, function() {
                sc_rectangle_is_slide_up = false;
            });
            $(window.parent.document).find('.sc_data_show label').animate({opacity: 0}, 200);
            $(window.parent.document).find('.sc_data_show').slideUp(500, function() {
                sc_rectangle_is_slide_up = false;
            });
        ...
        function sc_minimize() {
            $(window.parent.document).find('.sc_circle').show();
            $(window.parent.document).find('.sc_rectangle').hide();
            $(window.parent.document).find('.sc_buttons').hide(); // 优化回弹问题
        }

        function sc_switch_css() {
            sc_switch++;
            let sc_rectangle = $(window.parent.document).find('.sc_rectangle');
            let sc_item = $(window.parent.document).find('.sc_item');
            let sc_list = $(window.parent.document).find('.sc_list');
            let sc_data_show = $(window.parent.document).find('.sc_data_show');
            let sc_button_item = $(window.parent.document).find('.sc_button_item');
            ...
            if (sc_switch === 0) {
                ...
                $(window.parent.document).find('#sc_scrollbar_style').text(`
            .sc_rectangle::-webkit-scrollbar {
                width: 6px;
            }
            .sc_rectangle:hover::-webkit-scrollbar-thumb {
        ...
        function sc_model_change() {
            let sc_button_model = $(window.parent.document).find('.sc_button_model');
            // 删除定时器
        ...
        function updateTimestampDiff() {
            let sc_timestamp_item = $(window.parent.document).find('.sc_start_timestamp');
        ...
        function sc_fetch_and_show() {
            ...
            fetch(sc_url).then(response => {
                ...
                if (ret.code === 0) {
                    ...
                    $(window.parent.document).find('.sc_high_energy_num_right').text(high_energy_num);

                    // 舰长数
                    let captain_num = ret.data.guard_info.count;
                    $(window.parent.document).find('.sc_captain_num_right').text(captain_num);
                    ...
                    if (sc_add_arr.length > 0) {
                        for (let i = 0; i < sc_add_arr.length; i++){
                            ...
                            $(window.parent.document).find('.live-room-app .sc_list').prepend(sc_item_html);
