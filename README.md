...
# bilibili-sc-record
B站直播间SC记录板-实时同步SC、同接、高能和舰长数据，可拖拽移动，可导出，可单个SC折叠，可侧折，可搜索，可记忆配置，可生成图片（右键菜单），活动页可用，直播全屏可用，黑名单功能，不用登录，多种主题切换，自动清除超过12小时的房间SC存储，可自定义SC过期时间，可指定用户进入直播间提示、弹幕高亮和SC转弹幕，可让所有的实时SC以弹幕方式展现，可自动点击天选，可自动跟风发送combo弹幕
...
如果不想用油猴脚本，而想像Edge浏览器那样的扩展的（如chrome浏览器、360浏览器，但火狐浏览器不行），可以：
...
5、修改_locales/zh_CN目录里面的messages.json（看需要）
...
- 支持自定义搜索和快速定位SC
    ...
    >可以在右键菜单的【其他一些功能的自定义设置】设置开启/关闭搜索快捷键（默认开启）
    >
    >开启/关闭搜索弹窗：ctrl + f
    >
    >搜索上一个：ctrl + 左或上方向键（搜索弹窗关闭状态下也起作用）
    >
    >搜索下一个：ctrl + 右或下方向键（搜索弹窗关闭状态下也起作用）
...
- 支持自动点击天选
  ...
  >可以在右键菜单的【其他一些功能的自定义设置】设置开启/关闭（默认关闭）
  >
  >条件：当前直播间，并且已经关注主播
